...
End Sub
Sub prepareReadings(ByVal expectedRows As Integer, ByVal id As Integer)
    ...
    readingSheet.Range("B2").Value = id 'Range("D" & rawRow.Cells(1).Row).Value
    ...
    Do While currentRow < expectedRows
        ...
        Select Case shtReadings.Range("C" & currentRow).Value
            Case "I", "3"
                statusString = "Entry"
        ...
        End If
        codeASCII = codeASCII + 1
        readingSheet.Range("C" & currentRow).Value = statusString
        ...
        If (validityString = invalidString) Then
            ...
            If readingSheet.Range("C" & rowBegins + i).Value = "Exit" Then
                ...
                If (rowBegins + i > rowBegins) Then
                    ...
                    End If
                Else
                ' always assume the first entry of record is entry
                    readingSheet.Range("C" & rowBegins + i).Value = "Entry"
                    validateAgain = True
                    GoTo Changed
                End If
